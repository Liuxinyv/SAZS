...
def validate(val_loader, model, criterion, eval_score=None, print_freq=1,texts=None,lmodel=None):
    ...
    # writer = SummaryWriter(log_dir=os.path.join('runs',str(args.fold) + str(args.arch) + 'l' + str(args.lr)))
    for i, batch in enumerate(val_loader):
        input = batch['query_img']
        target = batch['query_mask']
        class_sample = batch['class_id']
        target, _ = extract_ignore_idx(target, class_sample)
        h, w = input.size()[2:4]
...
def extract_ignore_idx_color(mask, class_id):
    boundary = (mask / 255).floor()
    mask[mask == 255] = 0
    return mask, boundary
def train(args,train_loader,val_loader, model,criterion, optimizer, epoch,
          eval_score=None, print_freq=1000,texts=None,lmodel=None):
    batch_time = AverageMeter()
    ...
    model.train()
    # writer = SummaryWriter(log_dir='runs/lseg_coco_vit_f'+str(args.fold)+str(args.arch)+'l'+str(args.lr))
    end = time.time()
...
def train_seg(args):
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        val_miou,val_fb_iou = validate(val_loader, model, criterion, eval_score=accuracy,texts=texts,lmodel=lmodel)
        ...
        new_dir = os.path.join(args.filename,'d'+str(args.crop_size)+'_'+ args.arch+ '_f'+str(args.fold) +'_s' + str(args.lr) +'_B'+ str(args.batch_size)+ str(args.workers)+'_'+str(ran))
...
def fast_hist(pred, label,ignore=None):
    if ignore is not None:
...
def save_colorful_images(image,gt,predictions,label,pred, filenames, output_dir, palettes,mask):
    ...
    If given a mini-batch tensor, will save the tensor as a grid of images.
    """
    #原图，多类别gt,多类别vis,spectral-mask,二分类gt,二分类pred
    for ind in range(len(filenames)):
...
def test_sp(args, eval_data_loader, model, num_classes,
           has_gt=True, save_vis=False, texts=None,num_labels=None, lmodel=None):
    model.eval()
    batch_time = AverageMeter()
    data_time = AverageMeter()
    end = time.time()
    ...
    utils.fix_randseed(0)
    # for iter, (image, label,edgemap, class_sample, name) in enumerate(eval_data_loader):
...
def test_seg(args):
    ...
    mAP = test_sp(args, test_loader, model, 2, save_vis=False,
...
if args.cmd == 'train':
        logging.basicConfig(format=FORMAT, filename=os.path.join(args.filename + 'd' + str(
            ...
            args.batch_size)  + str(args.ran) + '.log'))
elif args.cmd == 'test':
    logging.basicConfig(format=FORMAT,filename=os.path.join(args.filename, '_d'+str(args.crop_size)+'_'+ args.arch+ '_f'+str(args.fold) +'_s' + str(args.lr) +'_B'+ str(args.batch_size)+'_'+str(args.ran)+'.log'))
