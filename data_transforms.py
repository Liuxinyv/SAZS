...
class RandomCrop(object):
    ...
    def __call__(self, image, label, *args):
        ...
        w, h = image.size
        tw, th = self.size
        padw = tw - w if w < tw else 0
        padh = th - h if h < th else 0
        img = ImageOps.expand(image, border=(0, 0, padw, padh), fill=0)
        mask = ImageOps.expand(label, border=(0, 0, padw, padh), fill=255)

        # cropping
        w, h = img.size
        x = random.randint(0, w - tw)
        y = random.randint(0, h - th)
        image = img.crop((x, y, x + tw, y + th))
        label = mask.crop((x, y, x + tw, y + th))
