...
class DatasetFSS(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # query_img = self.transform(query_img)#3*480*480
    ...
    def read_mask(self, img_name):
        # mask = torch.tensor(np.array(Image.open(img_name).convert('L')))#224*224
        mask = Image.open(img_name).convert('L')
        # mask[mask < 128] = 0
        # mask[mask >= 128] = 1
        return mask
        ...
        for cat in self.categories:
            ...
            if self.split == 'test' and self.shot == 1:
                ...
                for img_path in img_paths:
                    if os.path.basename(img_path).split('.')[1] == 'jpg':
                        img_metadata.append(img_path)
        # list = [str(i) for i in img_metadata]
        # list = '\n'.join(list)
